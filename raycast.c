...
/*   Updated: 2024/12/23 13:18:45 by arekoune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void horizontal_hit(double *hitp, int *der, t_player player, double angel)
{
    if(der[0] == -1)
        hitp[0] = floor(player.cord[0] / TAILE_SIZE) * TAILE_SIZE - 0.000001;
         ...
         hitp[0] = floor(player.cord[0] / TAILE_SIZE) * TAILE_SIZE + TAILE_SIZE + 0.000001;
    if(der[1] == 1)
...
void victical_hit(double *hitp, int *der, t_player player, double angel)
{
    if(der[1] == -1)
        hitp[1] = floor(player.cord[1] / TAILE_SIZE) * TAILE_SIZE - 0.000001;
         ...
         hitp[1] = floor(player.cord[1] / TAILE_SIZE) * TAILE_SIZE + TAILE_SIZE + 0.000001;
    if(der[0] == 1)
...
double  *hitpoint(t_map *map,double angel,double *hitph,double  *hitpv)
{
    ...
    {
        map->color_test = create_trgb(0, 144, 0, 255);
        return (hitpv);
    }
    else{
        map->color_test = create_trgb(255, 255, 0, 255);
        return (hitph);
    }
}
